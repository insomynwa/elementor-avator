<?php
namespace ElementorAvator\Core\Connect\Apps;

use Elementor\Core\Common\Modules\Connect\Apps\Common_App;
use ElementorAvator\License;
...
class Activate extends Common_App {
	public function get_title() {
		return __( 'Activate', 'elementor-avator' );
	}
	...
	public function action_activate_license() {
		if ( ! $this->is_connected() ) {
			$this->add_notice( __( 'Please connect to Elementor in order to activate license.', 'elementor-avator' ), 'error' );
		...
		if ( empty( $license ) ) {
			...
			wp_die( 'License not found for user ' . $this->get( 'user' )->email, __( 'Elementor Avator', 'elementor-avator' ), [
				'back_link' => true,
			] );
		}

		if ( is_wp_error( $license ) ) {
			wp_die( $license, __( 'Elementor Avator', 'elementor-avator' ), [
				'back_link' => true,
			] );
		}
		...
		if ( empty( $license_key ) ) {
			wp_die( __( 'License key is missing.', 'elementor-avator' ), __( 'Elementor Avator', 'elementor-avator' ), [
				'back_link' => true,
			] );
		}
		...
		$this->add_notice( __( 'License has been activated successfully.', 'elementor-avator' ) );
