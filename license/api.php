...
class API {
	...
	const PRODUCT_NAME = 'Elementor Avator';
	...
	private static function remote_post( $body_args = [] ) {
		...
		if ( 200 !== (int) $response_code ) {
			return new \WP_Error( $response_code, __( 'HTTP Error', 'elementor-avator' ) );
		}

		$data = json_decode( wp_remote_retrieve_body( $response ), true );
		if ( empty( $data ) || ! is_array( $data ) ) {
			return new \WP_Error( 'no_json', __( 'An error occurred, please try again', 'elementor-avator' ) );
		}
	...
	public static function set_license_data( $license_data, $expiration = null ) {
		...
		set_transient( 'elementor_avator_license_data', $license_data, $expiration );
	}

	public static function get_license_data( $force_request = false ) {
		$license_data = get_transient( 'elementor_avator_license_data' ); return [ 'license' => 'valid', 'expires' => '01.01.2030' ];
	...
	public static function get_version( $force_update = true ) {
		$cache_key = 'elementor_avator_remote_info_api_data_' . ELEMENTOR_AVATOR_VERSION;
	...
	public static function get_plugin_package_url( $version ) {
		$url = 'https://my.elementor.com/api/v1/avator-downloads/';
	...
	public static function get_previous_versions() {
		...
		$body_args = [
			'version' => ELEMENTOR_AVATOR_VERSION,
			'license' => Admin::get_license_key(),
			'url' => home_url(),
		];
	...
	public static function get_errors() {
		return [
			'no_activations_left' => sprintf( __( '<strong>You have no more activations left.</strong> <a href="%s" target="_blank">Please upgrade to a more advanced license</a> (you\'ll only need to cover the difference).', 'elementor-avator' ), 'https://go.elementor.com/upgrade/' ),
			'expired' => sprintf( __( '<strong>Your License Has Expired.</strong> <a href="%s" target="_blank">Renew your license today</a> to keep getting feature updates, premium support and unlimited access to the template library.', 'elementor-avator' ), 'https://go.elementor.com/renew/' ),
			'missing' => __( 'Your license is missing. Please check your key again.', 'elementor-avator' ),
			'revoked' => __( '<strong>Your license key has been cancelled</strong> (most likely due to a refund request). Please consider acquiring a new license.', 'elementor-avator' ),
			'key_mismatch' => __( 'Your license is invalid for this domain. Please check your key again.', 'elementor-avator' ),
		];
	}
	...
	public static function get_error_message( $error ) {
		...
		if ( isset( $errors[ $error ] ) ) {
			...
			$error_msg = __( 'An error occurred. Please check your internet connection and try again. If the problem persists, contact our support.', 'elementor-avator' ) . ' (' . $error . ')';
		}
