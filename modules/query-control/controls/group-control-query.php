...
class Group_Control_Query extends Group_Control_Base {
	...
	protected function init_fields_by_name( $name ) {
		...
		$fields['post_type'] = [
			'label' => __( 'Source', 'elementor-avator' ),
			'type' => Controls_Manager::SELECT,
			'options' => [
				'by_id' => __( 'Manual Selection', 'elementor-avator' ),
				'current_query' => __( 'Current Query', 'elementor-avator' ),
			],
		];
		...
		$fields['query_include'] = [
			...
			'label' => __( 'Include', 'elementor-avator' ),
		...
		$fields['posts_ids'] = [
			'label' => __( 'Search & Select', 'elementor-avator' ),
		...
		$fields['include'] = [
			'label' => __( 'Include By', 'elementor-avator' ),
			'type' => Controls_Manager::SELECT2,
			'multiple' => true,
			'options' => [
				'terms' => __( 'Term', 'elementor-avator' ),
				'authors' => __( 'Author', 'elementor-avator' ),
			],
			'condition' => [
				'post_type!' => [
					'by_id',
		...
		$fields['include_term_ids'] = [
			'label' => __( 'Term', 'elementor-avator' ),
			'description' => __( 'Terms are items in a taxonomy. The available taxonomies are: Categories, Tags, Formats and custom taxonomies.', 'elementor-avator' ),
			'type' => Query_Module::QUERY_CONTROL_ID,
			'options' => [],
			'label_block' => true,
			'multiple' => true,
		...
		$fields['query_exclude'] = [
			'type' => Controls_Manager::TAB,
			'label' => __( 'Exclude', 'elementor-avator' ),
			'tabs_wrapper' => $tabs_wrapper,
			'condition' => [
				'post_type!' => [
					'by_id',
		...
		$fields['exclude'] = [
			'label' => __( 'Exclude By', 'elementor-avator' ),
			'type' => Controls_Manager::SELECT2,
			'multiple' => true,
			'options' => [
				'current_post' => __( 'Current Post', 'elementor-avator' ),
				'manual_selection' => __( 'Manual Selection', 'elementor-avator' ),
				'terms' => __( 'Term', 'elementor-avator' ),
				'authors' => __( 'Author', 'elementor-avator' ),
			],
			'condition' => [
		...
		$fields['avoid_duplicates'] = [
			'label' => __( 'Avoid Duplicates', 'elementor-avator' ),
			'type' => Controls_Manager::SWITCHER,
			'default' => '',
			'description' => __( 'Set to Yes to avoid duplicate posts from showing up. This only effects the frontend.', 'elementor-avator' ),
			'tabs_wrapper' => $tabs_wrapper,
			'inner_tab' => $exclude_wrapper,
			'condition' => [
				'post_type!' => [
		...
		$fields['offset'] = [
			...
			'description' => __( 'Use this setting to skip over posts (e.g. \'2\' to skip over 2 posts).', 'elementor-avator' ),
			'tabs_wrapper' => $tabs_wrapper,
			'inner_tab' => $exclude_wrapper,
		];

		$fields['select_date'] = [
			'label' => __( 'Date', 'elementor-avator' ),
			'type' => Controls_Manager::SELECT,
			'post_type' => '',
			'options' => [
				'anytime' => __( 'All', 'elementor-avator' ),
				'today' => __( 'Past Day', 'elementor-avator' ),
				'week' => __( 'Past Week', 'elementor-avator' ),
				'month'  => __( 'Past Month', 'elementor-avator' ),
				'quarter' => __( 'Past Quarter', 'elementor-avator' ),
				'year' => __( 'Past Year', 'elementor-avator' ),
		...
		$fields['date_before'] = [
			'label' => __( 'Before', 'elementor-avator' ),
			'type' => Controls_Manager::DATE_TIME,
			'post_type' => '',
			'label_block' => false,
			'multiple' => false,
			'placeholder' => __( 'Choose', 'elementor-avator' ),
			'condition' => [
				'select_date' => 'exact',
				'post_type!' => [
					'by_id',
			...
			'description' => __( 'Setting a ‘Before’ date will show all the posts published until the chosen date (inclusive).', 'elementor-avator' ),
		];

		$fields['date_after'] = [
			'label' => __( 'After', 'elementor-avator' ),
			'type' => Controls_Manager::DATE_TIME,
			'post_type' => '',
			'label_block' => false,
			'multiple' => false,
			...
			'description' => __( 'Setting an ‘After’ date will show all the posts published since the chosen date (inclusive).', 'elementor-avator' ),
		];
		...
		$fields['orderby'] = [
			...
			'default' => 'post_date',
			'options' => [
				'post_date' => __( 'Date', 'elementor-avator' ),
				'post_title' => __( 'Title', 'elementor-avator' ),
		...
		$fields['posts_per_page'] = [
			'label' => __( 'Posts Per Page', 'elementor-avator' ),
			'type' => Controls_Manager::NUMBER,
			'default' => 3,
			'condition' => [
				'post_type!' => 'current_query',
		...
		$fields['ignore_sticky_posts'] = [
			'label' => __( 'Ignore Sticky Posts', 'elementor-avator' ),
			'type' => Controls_Manager::SWITCHER,
			'default' => 'yes',
			'condition' => [
				'post_type' => 'post',
			],
			'description' => __( 'Sticky-posts ordering is visible on frontend only', 'elementor-avator' ),
		];

		$fields['query_id'] = [
			'label' => __( 'Query ID', 'elementor-avator' ),
			'type' => Controls_Manager::TEXT,
			'default' => '',
			'description' => __( 'Give your Query a custom unique id to allow server side filtering', 'elementor-avator' ),
			'separator' => 'before',
		];
