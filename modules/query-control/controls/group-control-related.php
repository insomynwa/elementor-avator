...
class Group_Control_Related extends Group_Control_Query {
	...
	protected function init_fields_by_name( $name ) {
		...
		$fields['post_type']['options']['related'] = __( 'Related', 'elementor-avator' );
		...
		$related_fallback = [
			'label' => __( 'Fallback', 'elementor-avator' ),
			'type' => Controls_Manager::SELECT,
			'options' => [
				'fallback_none' => __( 'None', 'elementor-avator' ),
				'fallback_by_id' => __( 'Manual Selection', 'elementor-avator' ),
				'fallback_recent' => __( 'Recent Posts', 'elementor-avator' ),
			],
			'default' => 'fallback_none',
			'description' => __( 'Displayed if no relevant results are found. Manual selection display order is random', 'elementor-avator' ),
			'condition' => [
				'post_type' => 'related',
			],
			'separator' => 'before',
