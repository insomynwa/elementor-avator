<?php
namespace ElementorAvator\Modules\QueryControl;

use Elementor\Controls_Manager;
use Elementor\Core\Common\Modules\Ajax\Module as Ajax;
use Elementor\Widget_Base;
use ElementorAvator\Base\Module_Base;
use ElementorAvator\Modules\QueryControl\Controls\Group_Control_Posts;
use ElementorAvator\Modules\QueryControl\Controls\Group_Control_Query;
use ElementorAvator\Modules\QueryControl\Controls\Group_Control_Related;
use ElementorAvator\Modules\QueryControl\Classes\Elementor_Post_Query;
use ElementorAvator\Modules\QueryControl\Classes\Elementor_Related_Query;
use ElementorAvator\Modules\QueryControl\Controls\Query;
...
class Module extends Module_Base {
	...
	public static function add_exclude_controls( $widget ) {
		...
		$widget->add_control(
			...
			[
				...
				'default' => '',
				'description' => __( 'Set to Yes to avoid duplicate posts from showing up on the page. This only affects the frontend.', 'elementor-avator' ),
			]
		);
	...
	public function ajax_posts_filter_autocomplete_deprecated( $data ) {
		...
		switch ( $data['filter_type'] ) {
			...
			case 'author':
				...
				break;
			default:
				$results = apply_filters( 'elementor_avator/query_control/get_autocomplete/' . $data['filter_type'], [], $data );
		}
	...
	public function ajax_posts_control_value_titles_deprecated( $request ) {
		...
		switch ( $request['filter_type'] ) {
			...
			case 'author':
				...
				break;
			default:
				$results = apply_filters( 'elementor_avator/query_control/get_value_titles/' . $request['filter_type'], [], $request );
		}
	...
	/**
	 * @param \ElementorAvator\Base\Base_Widget $widget
	 * @param string $name
	...
	public function register_ajax_actions( $ajax_manager ) {
		...
		$ajax_manager->register_ajax_action( 'avator_panel_posts_control_filter_autocomplete', [ $this, 'ajax_posts_filter_autocomplete' ] );
		/**
		 * @deprecated 2.6.0 use new `autocomplete` format
		 */
		$ajax_manager->register_ajax_action( 'query_control_value_titles_deprecated', [ $this, 'ajax_posts_control_value_titles_deprecated' ] );
		$ajax_manager->register_ajax_action( 'avator_panel_posts_control_filter_autocomplete_deprecated', [ $this, 'ajax_posts_filter_autocomplete_deprecated' ] );
	}

	public function localize_settings( $settings ) {
		$settings = array_replace_recursive( $settings, [
			'i18n' => [
				'all' => __( 'All', 'elementor-avator' ),
			],
		] );
