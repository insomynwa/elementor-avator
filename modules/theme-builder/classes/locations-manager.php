...
class Locations_Manager {
		...
		if ( $post_id ) {
			...
			if ( $document ) {
				...
				if ( $location_settings && ! $location_settings['edit_in_content'] ) {
					$content = '<div class="elementor-theme-builder-content-area">' . __( 'Content Area', 'elementor-avator' ) . '</div>';
				}
			}
		}
	...
	public function register_core_location( $location, $args = [] ) {
		if ( ! isset( $this->core_locations[ $location ] ) ) {
			...
			wp_die( esc_html( sprintf( __( 'Location \'%s\' is not a core location.', 'elementor-avator' ), $location ) ) );
		}
