...
use ElementorAvator\Modules\ThemeBuilder\Conditions\Condition_Base;
...
class Conditions_Manager {
	...
	public function admin_columns_headers( $posts_columns ) {
		...
		$posts_columns = array_slice( $posts_columns, 0, $offset, true ) + [
			'instances' => __( 'Instances', 'elementor-avator' ),
	...
	public function register_ajax_actions( $ajax_manager ) {
		$ajax_manager->register_ajax_action( 'avator_theme_builder_save_conditions', [ $this, 'ajax_save_theme_template_conditions' ] );
		$ajax_manager->register_ajax_action( 'avator_theme_builder_conditions_check_conflicts', [ $this, 'ajax_check_conditions_conflicts' ] );
	}

	public function ajax_check_conditions_conflicts( $request ) {
		$post_id = $request['editor_post_id'];

		/** @var \ElementorAvator\Modules\ThemeBuilder\Module $theme_builder_module */
		$theme_builder_module = Module::instance();
		...
		if ( ! empty( $conflicted ) ) {
			$message = __( 'Elementor recognized that you have set this location for other templates: ', 'elementor-avator' ) . ' ' . implode( ', ', $conflicted );
		}
	...
	private function register_condition( $id, $args = [] ) {
		...
		$class_name = '\\ElementorAvator\\Modules\\ThemeBuilder\\Conditions\\' . $class_name;
