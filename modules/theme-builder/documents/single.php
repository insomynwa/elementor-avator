...
class Single extends Theme_Page_Document {
	...
	public static function get_properties() {
		...
		$propertyes['location'] = 'single';
		$propertyes['condition_type'] = 'singular';
	...
	public static function get_title() {
		return __( 'Single', 'elementor-avator' );
	}
	...
	protected static function get_editor_panel_categories() {
		$categories = [
			'theme-elements-single' => [
				'title' => __( 'Single', 'elementor-avator' ),
			],
		];
	...
	public function print_content() {
		...
		if ( $requested_post_id !== $this->post->ID ) {
			...
			if ( $requested_document && ! $requested_document instanceof Section && $requested_document->get_location() !== $this->get_location() ) {
				echo '<div class="elementor-theme-builder-content-area">' . __( 'Content Area', 'elementor-avator' ) . '</div>';
	...
	public static function get_preview_as_options() {
		...
		return [
			'single' => [
				'label' => __( 'Single', 'elementor-avator' ),
				'options' => $post_types_options,
			],
			'page/404' => __( '404', 'elementor-avator' ),
		];
	}
	...
	public function preview_error_handler() {
		...
		wp_localize_script( 'elementor-frontend', 'elementorPreviewErrorArgs', [
			...
			'headerMessage' => sprintf( __( 'The %s Widget was not found in your template.', 'elementor-avator' ), $depended_widget_title ),
			/* translators: %1$s: is the widget name. %2$s: is the template name.  */
			'message' => sprintf( __( 'You must include the %1$s Widget in your template (%2$s), in order for Elementor to work on this page.', 'elementor-avator' ), $depended_widget_title, '<strong>' . static::get_title() . '</strong>' ),
			'strings' => [
				'confirm' => __( 'Edit Template', 'elementor-avator' ),
			],
