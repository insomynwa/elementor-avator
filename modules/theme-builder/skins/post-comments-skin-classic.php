...
class Post_Comments_Skin_Classic extends Skin_Base {
	...
	public function register_controls() {
		...
		$this->add_control(
			'show_gravatar',
			[
				'label' => __( 'Gravatar', 'elementor-avator' ),
		...
		$this->start_controls_section(
			...
			[
				'label' => __( 'Reply Button', 'elementor-avator' ),
				'tab' => Controls_Manager::TAB_STYLE,
			]
		);
		...
		<article id="div-comment-<?php comment_ID(); ?>" class="comment-body">
			<footer class="comment-meta">
				<div class="comment-author vcard">
					...
					printf( __( '%s <span class="says">says:</span>', 'elementor-avator' ),
				...
				<div class="comment-metadata">
					<a href="<?php echo esc_url( get_comment_link( $comment, $args ) ); ?>">
						<time datetime="<?php comment_time( 'c' ); ?>">
							...
							printf( __( '%1$s at %2$s', 'elementor-avator' ), get_comment_date( '', $comment ), get_comment_time() );
							?>
						</time>
					</a>
					<?php edit_comment_link( __( 'Edit', 'elementor-avator' ), '<span class="edit-link">', '</span>' ); ?>
				</div><!-- .comment-metadata -->

				<?php if ( '0' == $comment->comment_approved ) : ?>
					<p class="comment-awaiting-moderation"><?php esc_html_e( 'Your comment is awaiting moderation.', 'elementor-avator' ); ?></p>
				<?php endif; ?>
			</footer><!-- .comment-meta -->
