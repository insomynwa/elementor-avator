...
class Drip extends Integration_Base {
	...
	const OPTION_NAME_API_KEY = 'avator_drip_api_token';
	...
	public function get_label() {
		return __( 'Drip', 'elementor-avator' );
	}
	...
	public function register_settings_section( $widget ) {
		$widget->start_controls_section(
			'section_drip',
			[
				'label' => __( 'Drip', 'elementor-avator' ),
				'condition' => [
		...
		$widget->add_control(
			...
			[
				'label' => __( 'API Token', 'elementor-avator' ),
		...
		$widget->add_control(
			...
			[
				'label' => __( 'Custom API Token', 'elementor-avator' ),
				'type' => Controls_Manager::TEXT,
				'condition' => [
					'drip_api_token_source' => 'custom',
				],
				'description' => __( 'Use this field to set a custom API token for the current form', 'elementor-avator' ),
			]
		);
		...
		$widget->add_control(
			'drip_account',
			[
				'label' => __( 'Account', 'elementor-avator' ),
				'type' => Controls_Manager::SELECT,
				'options' => [],
				'render_type' => 'none',
				'conditions' => [
		...
		$widget->add_control(
			'drip_fields_map',
			[
				'label' => __( 'Email Field Mapping', 'elementor-avator' ),
		...
		$widget->add_control(
			...
			[
				'label' => __( 'Send Additional Data to Drip', 'elementor-avator' ),
		...
		$widget->add_control(
			...
			[
				...
				'default' => 'no',
				'description' => __( 'Send all form fields to drip as custom fields', 'elementor-avator' ),
				'condition' => [
					'drip_account!' => '',
				],
			]
	...
	public function run( $record, $ajax_handler ) {
		...
		if ( ! $subscriber ) {
			$ajax_handler->add_admin_error_message( __( 'Drip Integration requires an email field', 'elementor-avator' ) );
	...
	private function get_meta_data( $meta_data ) {
		...
		foreach ( $meta_data as $meta_type ) {
			switch ( $meta_type ) {
				...
				case 'credit':
					$custom_fields[ $meta_type ] = sprintf( __( 'Powered by %s', 'elementor-avator' ), 'https://elementor.com/pro/' );
					break;
			}
		}
	...
	public function register_admin_fields( Settings $settings ) {
		$settings->add_section( Settings::TAB_INTEGRATIONS, 'drip', [
			'callback' => function() {
				echo '<hr><h2>' . esc_html__( 'Drip', 'elementor-avator' ) . '</h2>';
			},
			'fields' => [
				self::OPTION_NAME_API_KEY => [
					...
					'field_args' => [
						'type' => 'text',
						'desc' => sprintf( __( 'To integrate with our forms you need an <a href="%s" target="_blank">API Key</a>.', 'elementor-avator' ), 'http://kb.getdrip.com/general/where-can-i-find-my-api-token/' ),
					],
				],
				'validate_api_data' => [
					'field_args' => [
						'type' => 'raw_html',
						'html' => sprintf( '<button data-action="%s" data-nonce="%s" class="button elementor-button-spinner" id="elementor_avator_drip_api_token_button">%s</button>', self::OPTION_NAME_API_KEY . '_validate', wp_create_nonce( self::OPTION_NAME_API_KEY ), __( 'Validate API Token', 'elementor-avator' ) ),
					],
				],
			],
		] );
