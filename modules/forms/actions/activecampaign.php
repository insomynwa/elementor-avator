...
class Activecampaign extends Classes\Integration_Base {
	...
	const OPTION_NAME_API_KEY = 'avator_activecampaign_api_key';
	const OPTION_NAME_API_URL = 'avator_activecampaign_api_url';
	...
	public function get_label() {
		return __( 'ActiveCampaign', 'elementor-avator' );
	}
	...
	public function register_settings_section( $widget ) {
		$widget->start_controls_section(
			...
			[
				'label' => __( 'ActiveCampaign', 'elementor-avator' ),
				'condition' => [
		...
		$widget->add_control(
			...
			[
				'label' => __( 'API Credentials', 'elementor-avator' ),
		...
		$widget->add_control(
			...
			[
				'label' => __( 'API Key', 'elementor-avator' ),
				'type' => Controls_Manager::TEXT,
				'description' => __( 'Use this field to set a custom API Key for the current form', 'elementor-avator' ),
				'condition' => [
		...
		$widget->add_control(
			...
			[
				'label' => __( 'API URL', 'elementor-avator' ),
				'type' => Controls_Manager::TEXT,
				'description' => __( 'Use this field to set a custom API URL for the current form', 'elementor-avator' ),
				'condition' => [
					'activecampaign_api_credentials_source' => 'custom',
				],
			]
		...
		$widget->add_control(
			...
			[
				'label' => __( 'Field Mapping', 'elementor-avator' ),
		...
		$widget->add_control(
			'activecampaign_tags',
			[
				'label' => __( 'Tags', 'elementor-avator' ),
				'type' => Controls_Manager::TEXT,
				'description' => __( 'Add as many tags as you want, comma separated.', 'elementor-avator' ),
				'condition' => [
					'activecampaign_list!' => '',
				],
			]
	...
	public function run( $record, $ajax_handler ) {
		...
		if ( ! $subscriber ) {
			$ajax_handler->add_admin_error_message( __( 'ActiveCampaign Integration requires an email field and a selected list', 'elementor-avator' ) );
	...
	public function register_admin_fields( Settings $settings ) {
		$settings->add_section( Settings::TAB_INTEGRATIONS, 'activecampign', [
			'callback' => function() {
				echo '<hr><h2>' . esc_html__( 'ActiveCampaign', 'elementor-avator' ) . '</h2>';
			},
			'fields' => [
				...
				self::OPTION_NAME_API_URL => [
					...
					'field_args' => [
						'type' => 'url',
						'desc' => sprintf( __( 'To integrate with our forms you need an <a href="%s" target="_blank">API Key</a>.', 'elementor-avator' ), 'https://help.activecampaign.com/hc/en-us/articles/207317590-Getting-started-with-the-API' ),
					],
				],
				'validate_api_data' => [
					'field_args' => [
						'type' => 'raw_html',
						'html' => sprintf( '<button data-action="%s" data-nonce="%s" class="button elementor-button-spinner" id="elementor_avator_activecampaign_api_key_button">%s</button>', self::OPTION_NAME_API_KEY . '_validate', wp_create_nonce( self::OPTION_NAME_API_KEY ), __( 'Validate API Key', 'elementor-avator' ) ),
					],
				],
			],
		] );
