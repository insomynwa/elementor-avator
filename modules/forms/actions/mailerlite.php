...
use ElementorAvator\Modules\Forms\Classes\Mailerlite_Handler;
...
class Mailerlite extends Integration_Base {
	...
	const OPTION_NAME_API_KEY = 'avator_mailerlite_api_key';
	...
	public function get_label() {
		return __( 'MailerLite', 'elementor-avator' );
	}
	...
	public function register_settings_section( $widget ) {
		$widget->start_controls_section(
			'section_mailerlite',
			[
				'label' => __( 'MailerLite', 'elementor-avator' ),
				'condition' => [
		...
		$widget->add_control(
			'mailerlite_group',
			[
				'label' => __( 'Group', 'elementor-avator' ),
				'type' => Controls_Manager::SELECT,
				'options' => [],
				'render_type' => 'none',
				'conditions' => [
		...
		$widget->add_control(
			'allow_resubscribe',
			[
				'label' => __( 'Allow Resubscribe', 'elementor-avator' ),
	...
	public function run( $record, $ajax_handler ) {
		...
		if ( ! $subscriber ) {
			$ajax_handler->add_admin_error_message( __( 'MailerLite Integration requires an email field', 'elementor-avator' ) );
			return;
		}
	...
	public function register_admin_fields( Settings $settings ) {
		$settings->add_section( Settings::TAB_INTEGRATIONS, 'mailerlite', [
			'callback' => function() {
				echo '<hr><h2>' . esc_html__( 'MailerLite', 'elementor-avator' ) . '</h2>';
			},
			'fields' => [
				self::OPTION_NAME_API_KEY => [
					...
					'field_args' => [
						'type' => 'text',
						'desc' => sprintf( __( 'To integrate with our forms you need an <a href="%s" target="_blank">API Key</a>.', 'elementor-avator' ), 'https://help.mailerlite.com/article/show/35040-where-can-i-find-the-api-key' ),
					],
				],
				'validate_api_data' => [
					'field_args' => [
						'type' => 'raw_html',
						'html' => sprintf( '<button data-action="%s" data-nonce="%s" class="button elementor-button-spinner" id="elementor_avator_mailerlite_api_key_button">%s</button>', self::OPTION_NAME_API_KEY . '_validate', wp_create_nonce( self::OPTION_NAME_API_KEY ), __( 'Validate API Key', 'elementor-avator' ) ),
					],
				],
			],
		] );
