...
class Getresponse extends Integration_Base {
	...
	const OPTION_NAME_API_KEY = 'avator_getresponse_api_key';
	...
	public function get_label() {
		return __( 'GetResponse', 'elementor-avator' );
	}
	...
	public function register_settings_section( $widget ) {
		$widget->start_controls_section(
			'section_getresponse',
			[
				'label' => __( 'GetResponse', 'elementor-avator' ),
				'condition' => [
		...
		$widget->add_control(
			...
			[
				'label' => __( 'Day Of Cycle', 'elementor-avator' ),
	...
	public function run( $record, $ajax_handler ) {
		...
		if ( ! $subscriber ) {
			$ajax_handler->add_admin_error_message( __( 'GetResponse Integration requires an email field', 'elementor-avator' ) );
	...
	public function register_admin_fields( Settings $settings ) {
		$settings->add_section( Settings::TAB_INTEGRATIONS, 'getresponse', [
			'callback' => function() {
				echo '<hr><h2>' . esc_html__( 'GetResponse', 'elementor-avator' ) . '</h2>';
			},
			'fields' => [
				self::OPTION_NAME_API_KEY => [
					...
					'field_args' => [
						'type' => 'text',
						'desc' => sprintf( __( 'To integrate with our forms you need an <a href="%s" target="_blank">API Key</a>.', 'elementor-avator' ), 'https://www.getresponse.com' ),
					],
				],
				'validate_api_data' => [
					'field_args' => [
						'type' => 'raw_html',
						'html' => sprintf( '<button data-action="%s" data-nonce="%s" class="button elementor-button-spinner" id="elementor_avator_getresponse_api_key_button">%s</button>', self::OPTION_NAME_API_KEY . '_validate', wp_create_nonce( self::OPTION_NAME_API_KEY ), __( 'Validate API Key', 'elementor-avator' ) ),
					],
				],
			],
		] );
