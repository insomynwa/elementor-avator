...
use ElementorAvator\Modules\Forms\Classes\Mailchimp_Handler;
...
class Mailchimp extends Integration_Base {
	...
	const OPTION_NAME_API_KEY = 'avator_mailchimp_api_key';
	...
	public function get_label() {
		return __( 'MailChimp', 'elementor-avator' );
	}
	...
	public function register_settings_section( $widget ) {
		$widget->start_controls_section(
			'section_mailchimp',
			[
				'label' => __( 'MailChimp', 'elementor-avator' ),
				'condition' => [
		...
		$widget->add_control(
			'mailchimp_api_key',
			[
				'label' => __( 'Custom API Key', 'elementor-avator' ),
		...
		$widget->add_control(
			'mailchimp_list',
			[
				'label' => __( 'Audience', 'elementor-avator' ),
				'type' => Controls_Manager::SELECT,
				'options' => [],
				'render_type' => 'none',
				'conditions' => [
		...
		$widget->add_control(
			'mailchimp_groups',
			[
				'label' => __( 'Groups', 'elementor-avator' ),
		...
		$widget->add_control(
			...
			[
				...
				'description' => __( 'Add comma separated tags', 'elementor-avator' ),
				'type' => Controls_Manager::TEXT,
				'render_type' => 'none',
				'condition' => [
					'mailchimp_list!' => '',
		...
		$widget->add_control(
			...
			[
				'label' => __( 'Double Opt-In', 'elementor-avator' ),
	...
	public function register_admin_fields( Settings $settings ) {
		$settings->add_section( Settings::TAB_INTEGRATIONS, 'mailchimp', [
			'callback' => function() {
				echo '<hr><h2>' . esc_html__( 'MailChimp', 'elementor-avator' ) . '</h2>';
			},
			'fields' => [
				self::OPTION_NAME_API_KEY => [
					...
					'field_args' => [
						'type' => 'text',
						'desc' => sprintf( __( 'To integrate with our forms you need an <a href="%s" target="_blank">API Key</a>.', 'elementor-avator' ), 'https://kb.mailchimp.com/integrations/api-integrations/about-api-keys' ),
					],
				],
				'validate_api_data' => [
					'field_args' => [
						'type' => 'raw_html',
						'html' => sprintf( '<button data-action="%s" data-nonce="%s" class="button elementor-button-spinner" id="elementor_avator_mailchimp_api_key_button">%s</button>', self::OPTION_NAME_API_KEY . '_validate', wp_create_nonce( self::OPTION_NAME_API_KEY ), __( 'Validate API Key', 'elementor-avator' ) ),
					],
				],
			],
		] );
