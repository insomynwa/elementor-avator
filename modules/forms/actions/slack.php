...
class Slack extends Action_Base {
	...
	public function get_label() {
		return __( 'Slack', 'elementor-avator' );
	}
	...
	public function register_settings_section( $widget ) {
		$widget->start_controls_section(
			'section_slack',
			[
				'label' => __( 'Slack', 'elementor-avator' ),
				'condition' => [
		...
		$widget->add_control(
			...
			[
				...
				'label_block' => true,
				'separator' => 'before',
				'description' => __( 'Enter the webhook URL that will receive the form\'s submitted data.', 'elementor-avator' ) . ' ' . sprintf( '<a href="%s" target="_blank">%s</a>.', 'https://slack.com/apps/A0F7XDUAZ-incoming-webhooks/', __( 'Click here for Instructions', 'elementor-avator' ) ),
				'render_type' => 'none',
		...
		$widget->add_control(
			'slack_channel',
			[
				'label' => __( 'Channel', 'elementor-avator' ),
				'type' => Controls_Manager::TEXT,
			]
		);
		...
		$widget->add_control(
			'slack_pretext',
			[
				'label' => __( 'Pre Text', 'elementor-avator' ),
				'type' => Controls_Manager::TEXT,
			]
		);
	...
	public function run( $record, $ajax_handler ) {
		...
		$attachment = [
			'text' => __( 'A new Form Submission has been received', 'elementor-avator' ),
			'title' => __( 'A new Submission', 'elementor-avator' ),
		...
		if ( ! empty( $settings['slack_add_ts'] ) && 'yes' === $settings['slack_add_ts'] ) {
			$attachment = array_merge( $attachment, [
				'footer' => sprintf( __( 'Powered by %s', 'elementor-avator' ), 'Elementor' ),
		...
		$webhook_data = apply_filters( 'elementor_avator/forms/slack/webhook_args', $webhook_data );
