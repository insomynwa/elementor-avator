...
class Discord extends Action_Base {
	...
	public function get_label() {
		return __( 'Discord', 'elementor-avator' );
	}
	...
	public function register_settings_section( $widget ) {
		$widget->start_controls_section(
			'section_discord',
			[
				'label' => __( 'Discord', 'elementor-avator' ),
				'condition' => [
		...
		$widget->add_control(
			'discord_webhook',
			[
				'label' => __( 'Webhook URL', 'elementor-avator' ),
				'type' => Controls_Manager::TEXT,
				'placeholder' => 'https://discordapp.com/api/webhooks/',
				'label_block' => true,
				'separator' => 'before',
				'description' => __( 'Enter the webhook URL that will receive the form\'s submitted data.', 'elementor-avator' ) . ' ' . sprintf( '<a href="%s" target="_blank">%s</a>.', 'https://support.discordapp.com/hc/en-us/articles/228383668-Intro-to-Webhooks', __( 'Click here for Instructions', 'elementor-avator' ) ),
				'render_type' => 'none',
			]
		);
		...
		$widget->add_control(
			'discord_avatar_url',
			[
				'label' => __( 'Avatar URL', 'elementor-avator' ),
				'type' => Controls_Manager::TEXT,
			]
		);
		...
		$widget->add_control(
			'discord_form_data',
			[
				'label' => __( 'Form Data', 'elementor-avator' ),
		...
		$widget->add_control(
			'discord_ts',
			[
				'label' => __( 'Timestamp', 'elementor-avator' ),
				'type' => Controls_Manager::SWITCHER,
				'default' => 'yes',
			]
		);
	...
	public function run( $record, $ajax_handler ) {
		...
		$embeds = [
			'title' => isset( $settings['discord_title'] ) ? $settings['discord_title'] : __( 'A new Submission', 'elementor-avator' ),
			'description' => isset( $settings['discord_content'] ) ? $settings['discord_content'] : __( 'A new Form Submission has been received', 'elementor-avator' ),
			'author' => [
				'name'     => isset( $settings['discord_username'] ) ? $settings['discord_username'] : __( 'Elementor Forms', 'elementor-avator' ),
				'url'      => $page_url,
		...
		if ( ! empty( $settings['discord_ts'] ) && 'yes' === $settings['discord_ts'] ) {
			...
			$embeds['footer'] = [
				'text' => sprintf( __( 'Powered by %s', 'elementor-avator' ), 'Elementor' ),
		...
		$webhook_data = apply_filters( 'elementor_avator/forms/discord/webhook_args', $webhook_data );
