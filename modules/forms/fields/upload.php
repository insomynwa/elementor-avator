<?php
namespace ElementorAvator\Modules\Forms\Fields;

use Elementor\Widget_Base;
use ElementorAvator\Modules\Forms\Classes;
use Elementor\Controls_Manager;
use ElementorAvator\Modules\Forms\Widgets\Form;
...
class Upload extends Field_Base {
	...
	public function get_name() {
		return __( 'File Upload', 'elementor-avator' );
	}
	...
	public function update_controls( $widget ) {
		...
		$field_controls = [
			'file_sizes' => [
				'name' => 'file_sizes',
				'label' => __( 'Max. File Size', 'elementor-avator' ),
				'type' => Controls_Manager::SELECT,
				'condition' => [
					'field_type' => $this->get_type(),
				],
				'options' => $this->get_upload_file_size_options(),
				'description' => __( 'If you need to increase max upload size please contact your hosting.', 'elementor-avator' ),
				'tab' => 'content',
			...
			'file_types' => [
				'name' => 'file_types',
				'label' => __( 'Allowed File Types', 'elementor-avator' ),
				'type' => Controls_Manager::TEXT,
				'condition' => [
					'field_type' => $this->get_type(),
				],
				'description' => __( 'Enter the allowed file types, separated by a comma (jpg, gif, pdf, etc).', 'elementor-avator' ),
				'tab' => 'content',
				'inner_tab' => 'form_fields_content_tab',
				'tabs_wrapper' => 'form_fields_tabs',
			],
			'allow_multiple_upload' => [
				...
				'label' => __( 'Multiple Files', 'elementor-avator' ),
			...
			'max_files' => [
				'name' => 'max_files',
				'label' => __( 'Max. Files', 'elementor-avator' ),
		...
		if ( ! empty( $item['file_sizes'] ) ) {
			$form->add_render_attribute(
				...
				[
					...
					'data-maxsize-message' => __( 'This file exceeds the maximum allowed size.', 'elementor-avator' ),
				]
			);
		}
	...
	private function get_blacklist_file_ext() {
		...
		if ( ! $blacklist ) {
			 ...
			 */
			$blacklist = apply_filters( 'elementor_avator/forms/filetypes/blacklist', $blacklist );
		}
	...
	public function validation( $field, Classes\Form_Record $record, Classes\Ajax_Handler $ajax_handler ) {
		...
		if ( ! $upload_errors ) {
			$upload_errors = [
				UPLOAD_ERR_OK => __( 'There is no error, the file uploaded with success.', 'elementor-avator' ),
				/* translators: 1: upload_max_filesize, 2: php.ini */
				UPLOAD_ERR_INI_SIZE => sprintf( __( 'The uploaded file exceeds the %1$s directive in %2$s.', 'elementor-avator' ), 'upload_max_filesize', 'php.ini' ),
				/* translators: %s: MAX_FILE_SIZE */
				UPLOAD_ERR_FORM_SIZE => sprintf( __( 'The uploaded file exceeds the %s directive that was specified in the HTML form.', 'elementor-avator' ), 'MAX_FILE_SIZE' ),
				UPLOAD_ERR_PARTIAL => __( 'The uploaded file was only partially uploaded.', 'elementor-avator' ),
				UPLOAD_ERR_NO_FILE => __( 'No file was uploaded.', 'elementor-avator' ),
				UPLOAD_ERR_NO_TMP_DIR => __( 'Missing a temporary folder.', 'elementor-avator' ),
				UPLOAD_ERR_CANT_WRITE => __( 'Failed to write file to disk.', 'elementor-avator' ),
				/* translators: %s: phpinfo() */
				UPLOAD_ERR_EXTENSION => sprintf( __( 'A PHP extension stopped the file upload. PHP does not provide a way to ascertain which extension caused the file upload to stop; examining the list of loaded extensions with %s may help.', 'elementor-avator' ), 'phpinfo()' ),
			];
		...
		if ( ! empty( $field['max_files'] ) ) {
			if ( count( $_FILES['form_fields'][ $id ] ) > $field['max_files'] ) {
				$error_message = sprintf(
					...
					_n( 'You can upload only %d file.', 'You can upload up to %d files.', intval( $field['max_files'] ), 'elementor-avator' ),
		...
		foreach ( $_FILES['form_fields'][ $id ] as $index => $file ) {
			...
			if ( ! $this->is_file_type_valid( $field, $file ) ) {
				$ajax_handler->add_error( $id, __( 'This file type is not allowed.', 'elementor-avator' ) );
			}

			// allowed file size?
			if ( ! $this->is_file_size_valid( $field, $file ) ) {
				$ajax_handler->add_error( $id, __( 'This file exceeds the maximum allowed size.', 'elementor-avator' ) );
			}
		}
	}
	...
	private function get_upload_dir() {
		 ...
		 */
		$path = apply_filters( 'elementor_avator/forms/upload_path', $path );
	...
	private function get_file_url( $file_name ) {
		 ...
		 */
		$url = apply_filters( 'elementor_avator/forms/upload_url', $url, $file_name );
		...
		foreach ( $_FILES['form_fields'][ $id ] as $index => $file ) {
			...
			if ( is_dir( $uploads_dir ) && is_writable( $uploads_dir ) ) {
				...
				if ( false !== $move_new_file ) {
					...
					$ajax_handler->add_error( $id, __( 'There was an error while trying to upload your file.', 'elementor-avator' ) );
				}
			} else {
				$ajax_handler->add_admin_error_message( __( 'Upload directory is not writable or does not exist.', 'elementor-avator' ) );
			}
		}
	}
	...
	public function __construct() {
		parent::__construct();
		add_action( 'elementor_avator/forms/process', [ $this, 'set_file_fields_values' ], 10, 2 );
	}
}
