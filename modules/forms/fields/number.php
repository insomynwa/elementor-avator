...
class Number extends Field_Base {
	...
	public function get_name() {
		return __( 'Number', 'elementor-avator' );
	}
	...
	public function update_controls( $widget ) {
		...
		$field_controls = [
			'field_min' => [
				'name' => 'field_min',
				'label' => __( 'Min. Value', 'elementor-avator' ),
			...
			'field_max' => [
				'name' => 'field_max',
				'label' => __( 'Max. Value', 'elementor-avator' ),
				'type' => Controls_Manager::NUMBER,
				'condition' => [
					'field_type' => $this->get_type(),
				],
	...
	public function validation( $field, Classes\Form_Record $record, Classes\Ajax_Handler $ajax_handler ) {
		...
		if ( ! empty( $field['field_max'] ) && $field['field_max'] < (int) $field['value'] ) {
			$ajax_handler->add_error( $field['id'], sprintf( __( 'The value must be less than or equal to %s', 'elementor-avator' ), $field['field_max'] ) );
		}

		if ( ! empty( $field['field_min'] ) && $field['field_min'] > (int) $field['value'] ) {
			$ajax_handler->add_error( $field['id'], sprintf( __( 'The value must be greater than or equal %s', 'elementor-avator' ), $field['field_min'] ) );
		}
	}
