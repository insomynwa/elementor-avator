...
class Form_Record {
	...
	public function get_formatted_data( $with_meta = false ) {
		$formatted = [];
		$no_label = __( 'No Label', 'elementor-avator' );
		$fields = $this->fields;
	...
	public function validate( $ajax_handler ) {
		foreach ( $this->fields as $id => $field ) {
			 ...
			 * @param Ajax_Handler $ajax_handler An instance of the ajax handler.
			 */
			do_action( "elementor_avator/forms/validation/{$field_type}", $field, $this, $ajax_handler );
		}
		 ...
		 * @param Ajax_Handler $ajax_handler An instance of the ajax handler.
		 */
		do_action( 'elementor_avator/forms/validation', $this, $ajax_handler );
	...
	public function process_fields( $ajax_handler ) {
		foreach ( $this->fields as $id => $field ) {
			 ...
			 * @param Ajax_Handler $ajax_handler An instance of the ajax handler.
			 */
			do_action( "elementor_avator/forms/process/{$field_type}", $field, $this, $ajax_handler );
		}
		 ...
		 * @param Ajax_Handler $ajax_handler An instance of the ajax handler.
		 */
		do_action( 'elementor_avator/forms/process', $this, $ajax_handler );
	}
	...
	private function set_meta() {
		...
		foreach ( $form_metadata as $metadata_type ) {
			switch ( $metadata_type ) {
				case 'date':
					$this->meta['date'] = [
						'title' => __( 'Date', 'elementor-avator' ),
				...
				case 'time':
					$this->meta['time'] = [
						'title' => __( 'Time', 'elementor-avator' ),
				...
				case 'page_url':
					$this->meta['page_url'] = [
						'title' => __( 'Page URL', 'elementor-avator' ),
				...
				case 'user_agent':
					$this->meta['user_agent'] = [
						'title' => __( 'User Agent', 'elementor-avator' ),
				...
				case 'remote_ip':
					$this->meta['remote_ip'] = [
						'title' => __( 'Remote IP', 'elementor-avator' ),
						'value' => Utils::get_client_ip(),
					];
					break;
				case 'credit':
					$this->meta['credit'] = [
						'title' => __( 'Powered by', 'elementor-avator' ),
						'value' => __( 'Elementor', 'elementor-avator' ),
					];
					break;
			}
		}
	...
	private function sanitize_field( $field, $value ) {
		...
		switch ( $field_type ) {
			...
			default:
				 ...
				 */
				$value = apply_filters( "elementor_avator/forms/sanitize/{$field_type}", $value, $field );
		}
