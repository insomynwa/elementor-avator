...
class Recaptcha_Handler {
	...
	const OPTION_NAME_SITE_KEY = 'elementor_avator_recaptcha_site_key';

	const OPTION_NAME_SECRET_KEY = 'elementor_avator_recaptcha_secret_key';

	const OPTION_NAME_RECAPTCHA_THRESHOLD = 'elementor_avator_recaptcha_threshold';
	...
	public static function get_setup_message() {
		return __( 'To use reCAPTCHA, you need to add the API Key and complete the setup process in Dashboard > Elementor > Settings > Integrations > reCAPTCHA.', 'elementor-avator' );
	}

	public function register_admin_fields( Settings $settings ) {
		$settings->add_section( Settings::TAB_INTEGRATIONS, static::get_recaptcha_name(), [
			'label' => __( 'reCAPTCHA', 'elementor-avator' ),
			'callback' => function () {
				echo sprintf( __( '<a href="%s" target="_blank">reCAPTCHA</a> is a free service by Google that protects your website from spam and abuse. It does this while letting your valid users pass through with ease.', 'elementor-avator' ), 'https://www.google.com/recaptcha/' );
			},
			'fields' => [
				'avator_recaptcha_site_key' => [
					'label' => __( 'Site Key', 'elementor-avator' ),
					'field_args' => [
						'type' => 'text',
					],
				],
				'avator_recaptcha_secret_key' => [
					'label' => __( 'Secret Key', 'elementor-avator' ),
					'field_args' => [
						'type' => 'text',
					],
				],
	...
	public function register_scripts() {
		...
		wp_register_script( $script_name, $src, [], ELEMENTOR_AVATOR_VERSION, true );
	}
	...
	public function validation( $record, $ajax_handler ) {
		...
		if ( empty( $_POST['g-recaptcha-response'] ) ) {
			$ajax_handler->add_error( $field['id'], __( 'The Captcha field cannot be blank. Please enter a value.', 'elementor-avator' ) );

			return;
		}

		$recaptcha_errors = [
			'missing-input-secret' => __( 'The secret parameter is missing.', 'elementor-avator' ),
			'invalid-input-secret' => __( 'The secret parameter is invalid or malformed.', 'elementor-avator' ),
			'missing-input-response' => __( 'The response parameter is missing.', 'elementor-avator' ),
			'invalid-input-response' => __( 'The response parameter is invalid or malformed.', 'elementor-avator' ),
		];
		...
		if ( 200 !== (int) $response_code ) {
			...
			$ajax_handler->add_error( $field['id'], sprintf( __( 'Can not connect to the reCAPTCHA server (%d).', 'elementor-avator' ), $response_code ) );
		...
		if ( ! $this->validate_result( $result, $field ) ) {
			$message = __( 'Invalid Form - reCAPTCHA validation failed', 'elementor-avator' );
	...
	public function add_field_type( $field_types ) {
		$field_types['recaptcha'] = __( 'reCAPTCHA', 'elementor-avator' );
	...
	public function __construct() {
		...
		add_filter( 'elementor_avator/forms/field_types', [ $this, 'add_field_type' ] );
		add_action( 'elementor_avator/forms/render_field/' . static::get_recaptcha_name(), [ $this, 'render_field' ], 10, 3 );
		add_filter( 'elementor_avator/forms/render/item', [ $this, 'filter_field_item' ] );
		add_filter( 'elementor_avator/editor/localize_settings', [ $this, 'localize_settings' ] );

		if ( static::is_enabled() ) {
			add_action( 'elementor_avator/forms/validation', [ $this, 'validation' ], 10, 2 );
