...
class Recaptcha_V3_Handler extends Recaptcha_Handler {
	...
	const OPTION_NAME_V3_SITE_KEY = 'elementor_avator_recaptcha_v3_site_key';
	const OPTION_NAME_V3_SECRET_KEY = 'elementor_avator_recaptcha_v3_secret_key';
	const OPTION_NAME_RECAPTCHA_THRESHOLD = 'elementor_avator_recaptcha_v3_threshold';
	const V3 = 'v3';
	...
	public static function get_setup_message() {
		return __( 'To use reCAPTCHA V3, you need to add the API Key and complete the setup process in Dashboard > Elementor > Settings > Integrations > reCAPTCHA V3.', 'elementor-avator' );
	}

	public function register_admin_fields( Settings $settings ) {
		$settings->add_section( Settings::TAB_INTEGRATIONS, 'recaptcha_v3', [
			'label' => __( 'reCAPTCHA V3', 'elementor-avator' ),
			'callback' => function() {
				echo sprintf( __( '<a href="%s" target="_blank">reCAPTCHA V3</a> is a free service by Google that protects your website from spam and abuse. It does this while letting your valid users pass through with ease.', 'elementor-avator' ), 'https://www.google.com/recaptcha/intro/v3.html' );
			},
			'fields' => [
				'avator_recaptcha_v3_site_key' => [
					'label' => __( 'Site Key', 'elementor-avator' ),
					'field_args' => [
						'type' => 'text',
					],
				],
				'avator_recaptcha_v3_secret_key' => [
					'label' => __( 'Secret Key', 'elementor-avator' ),
					'field_args' => [
						'type' => 'text',
					],
				],
				'avator_recaptcha_v3_threshold' => [
					'label' => __( 'Score Threshold', 'elementor-avator' ),
					'field_args' => [
						'attributes' => [
							'min' => 0,
							'max' => 1,
						...
						'std' => 0.5,
						'type' => 'number',
						'desc' => __( 'Score threshold should be a value between 0 and 1, default: 0.5', 'elementor-avator' ),
					],
				],
			],
		] );
	...
	protected function add_error( $ajax_handler, $field, $message ) {
		...
		$ajax_handler->add_error_message( __( 'reCAPTCHA V3 validation failed, suspected as abusive usage', 'elementor-avator' ) );
	}
	...
	public function add_field_type( $field_types ) {
		$field_types['recaptcha_v3'] = __( 'reCAPTCHA V3', 'elementor-avator' );
	...
	public function __construct() {
		parent::__construct();
		add_filter( 'elementor_avator/forms/render/item/' . self::get_recaptcha_name(), [ $this, 'filter_recaptcha_item' ], 10, 3 );
	}
}
