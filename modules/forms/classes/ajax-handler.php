...
class Ajax_Handler {
	...
	public static function is_form_submitted() {
		return wp_doing_ajax() && isset( $_POST['action'] ) && 'elementor_avator_forms_send_form' === $_POST['action'];
	}

	public static function get_default_messages() {
		return [
			self::SUCCESS => __( 'The form was sent successfully.', 'elementor-avator' ),
			self::ERROR => __( 'An error occured.', 'elementor-avator' ),
			self::FIELD_REQUIRED => __( 'This field is required.', 'elementor-avator' ),
			self::INVALID_FORM => __( 'There\'s something wrong. The form is invalid.', 'elementor-avator' ),
			self::SERVER_ERROR => __( 'Server error. Form not sent.', 'elementor-avator' ),
			self::SUBSCRIBER_ALREADY_EXISTS => __( 'Subscriber already exists.', 'elementor-avator' ),
		];
	}
	...
	public static function get_default_message( $id, $settings ) {
		...
		return isset( $default_messages[ $id ] ) ? $default_messages[ $id ] : __( 'Unknown', 'elementor-avator' );
	}
	...
	public function ajax_send_form() {
		 ...
		 */
		do_action( 'elementor_avator/forms/new_record', $record, $this );
	...
	public function send() {
		...
		if ( current_user_can( 'edit_post', $_POST['post_id'] ) && ! empty( $this->messages['admin_error'] ) ) {
			$this->add_admin_error_message( __( 'This Message is not visible for site visitors.', 'elementor-avator' ) );
	...
	public function __construct() {
		add_action( 'wp_ajax_elementor_avator_forms_send_form', [ $this, 'ajax_send_form' ] );
		add_action( 'wp_ajax_nopriv_elementor_avator_forms_send_form', [ $this, 'ajax_send_form' ] );
	}
}
