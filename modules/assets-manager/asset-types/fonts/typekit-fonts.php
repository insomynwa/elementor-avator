...
use ElementorAvator\Modules\AssetsManager\Classes\Font_Base;
...
class Typekit_Fonts extends Font_Base {
	...
	public function get_name() {
		return __( 'TypeKit Web Fonts by Adobe', 'elementor-avator' );
	}
	...
	private function fetch_typekit_data() {
		...
		if ( 200 !== $response_code ) {
			switch ( $response_code ) {
				case 404:
					$this->error = __( 'Project not found.', 'elementor-avator' );
					break;
				default:
		...
		if ( ! $response_body ) {
			$this->error = __( 'No project data was returned.', 'elementor-avator' );
	...
	public function handle_panel_request( array $data ) {
		...
		if ( ! $typekit_fonts || ! is_array( $typekit_fonts ) ) {
			throw new \Exception( __( 'Error with TypeKit fonts', 'elementor-avator' ) );
		}

		if ( ! in_array( $font_family, array_keys( $typekit_fonts ) ) ) {
			throw new \Exception( __( 'Font missing in Project', 'elementor-avator' ) );
		}
	...
	public function register_admin_fields( Settings $settings ) {
		...
		$button_label = __( 'Get Project ID', 'elementor-avator' );
		$found_label = '<span class="elementor-avator-typekit-count">{{count}}</span> ' . __( 'Fonts Families Found in project. Please note that typekit takes a few minutes to sync once you publish or update a project.', 'elementor-avator' );
		if ( $fonts && is_array( $fonts ) ) {
			$button_label = __( 'Sync Project', 'elementor-avator' );
		}
		$settings->add_section( Settings::TAB_INTEGRATIONS, 'typekit', [
			'callback' => function() {
				echo '<hr><h2>' . esc_html__( 'TypeKit Web Fonts by Adobe', 'elementor-avator' ) . '</h2>';
				esc_html_e( 'TypeKit partners with the world’s leading type foundries to bring thousands of beautiful fonts to designers every day.', 'elementor-avator' );
			},
			'fields' => [
				self::TYPEKIT_KIT_ID_OPTION_NAME => [
					'label' => __( 'Project ID', 'elementor-avator' ),
					'field_args' => [
						'type' => 'text',
						'desc' => sprintf( __( 'Enter Your <a href="%s" target="_blank">TypeKit Project ID</a>.', 'elementor-avator' ), 'https://fonts.adobe.com/typekit' ),
					],
					'setting_args' => [
				...
				'validate_api_data' => [
					'field_args' => [
						'type' => 'raw_html',
						'html' => sprintf( '<button data-found="%s" data-action="%s" data-nonce="%s" class="button elementor-button-spinner" id="elementor_avator_typekit_validate_button">%s</button><br><p><span class="elementor-avator-typekit-data hidden"></span></p>',
	...
	protected function actions() {
		...
		add_action( 'wp_ajax_elementor_avator_admin_fetch_fonts', [ $this, 'integrations_admin_ajax_handler' ] );
	}
}
