<?php
namespace ElementorAvator\Modules\AssetsManager\AssetTypes\Fonts;

use Elementor\Core\Files\Assets\Files_Upload_Handler;
use Elementor\Core\Files\CSS\Base;
use ElementorAvator\Modules\AssetsManager\Classes;
use ElementorAvator\Modules\AssetsManager\AssetTypes\Fonts_Manager;
...
class Custom_Fonts extends Classes\Font_Base {
	...
	public function get_name() {
		return __( 'Custom Fonts', 'elementor-avator' );
	}
	...
	public function add_meta_box() {
		add_meta_box(
			...
			__( 'Manage Your Font Files', 'elementor-avator' ),
			[ $this, 'render_metabox' ],
			Fonts_Manager::CPT,
			'normal',
			'default'
	...
	public function render_metabox( $post ) {
		...
		$fields = [
			...
			[
				...
				'field_type' => 'html',
				'label' => false,
				'raw_html' => sprintf( '<div class="inline-preview">%s</div>', __( 'Elementor Is Making the Web Beautiful!!!', 'elementor-avator' ) ),
			],
			[
				'id' => 'toolbar',
				'field_type' => 'toolbar',
		...
		foreach ( $this->get_file_types() as $type => $mine ) {
			$fields[] = [
				...
				'label' => sprintf( __( '%s File', 'elementor-avator' ), strtoupper( $type ) ),
				/* translators: %s: Font file format. */
				'box_title' => sprintf( __( 'Upload font .%s file', 'elementor-avator' ), $type ),
				/* translators: %s: Font file format. */
				'box_action' => sprintf( __( 'Select .%s file', 'elementor-avator' ), $type ),
				'preview_anchor' => 'none',
		...
		$repeater = [
			...
			'id' => 'font_face',
			'label' => false,
			'add_label' => __( 'Add Font Variation', 'elementor-avator' ),
			'toggle_title' => __( 'Edit', 'elementor-avator' ),
			'remove_title' => __( 'Delete', 'elementor-avator' ),
			'field_type' => 'repeater',
			'row_label' => [
				'default' => 'Settings',
	...
	public function wp_handle_upload_prefilter( $file ) {
		...
		if ( Files_Upload_Handler::file_sanitizer_can_run() && ! $svg_handler->sanitize_svg( $file['tmp_name'] ) ) {
			$file['error'] = __( 'Invalid SVG Format, file not uploaded for security reasons', 'elementor-avator' );
		}
	...
	public function get_font_face_from_data( $font_family, $data ) {
		...
		$font_face .= "\tfont-display: " . apply_filters( 'elementor_avator/custom_fonts/font_display', 'auto', $font_family, $data ) . ';' . PHP_EOL;
	...
	public function handle_panel_request( array $data ) {
		...
		if ( empty( $font_face ) ) {
			...
			$error_message = sprintf( __( 'Font %s was not found.', 'elementor-avator' ), $font_family );
	...
	private function get_font_style_options() {
		return [
			...
			'italic' => __( 'Italic', 'elementor-avator' ),
			'oblique' => __( 'Oblique', 'elementor-avator' ),
		];
	}
	...
	private function get_file_type_description( $file_type ) {
		$descriptions = [
			'eot' => __( 'Embedded OpenType, Used by IE6-IE9 Browsers', 'elementor-avator' ),
			'woff2' => __( 'The Web Open Font Format 2, Used by Super Modern Browsers', 'elementor-avator' ),
			'woff' => __( 'The Web Open Font Format, Used by Modern Browsers', 'elementor-avator' ),
			'ttf' => __( 'TrueType Fonts, Used for better supporting Safari, Android, iOS', 'elementor-avator' ),
			'svg' => __( 'SVG fonts allow SVG to be used as glyphs when displaying text, Used by Legacy iOS', 'elementor-avator' ),
		];
