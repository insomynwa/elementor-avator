...
class Custom_Icons extends  Assets_Base {
	...
	public function get_name() {
		return __( 'Custom Icons', 'elementor-avator' );
	}
	...
	public function render_metabox( $post ) {
		...
		$fields = [
			...
			[
				...
				'label' => false,
				'sub-label' => __( 'Your Fontello, IcoMoon or Fontastic .zip file', 'elementor-avator' ),
			],
			[
				'id' => 'close_div',
				'field_type' => 'html_tag',
	...
	public static function get_supported_icon_sets() {
		...
		return array_merge( apply_filters( 'elementor_avator/icons_manager/custom_icons/additional_supported_types', [] ), $icon_sets );
	}
	...
	private function upload() {
		...
		if ( 'zip' !== $ext ) {
			...
			return new \WP_Error( 'unsupported_file', __( 'Only zip files are allowed', 'elementor-avator' ) );
		}
	...
	private function extract_zip( $file, $to ) {
		...
		if ( ! $unzip_result ) {
			$unzip_result = new \WP_Error( 'error', __( 'Could not unzip or empty archive.', 'elementor-avator' ) );
		}
	...
	private function upload_and_extract_zip() {
		...
		if ( count( $source_files ) === 0 ) {
			return new \WP_Error( 'incompatible_archive', esc_html__( 'Incompatible archive', 'elementor-avator' ) );
		}
	...
	public function custom_icons_upload_handler( $data ) {
		...
		return new \WP_Error( 'unsupported_zip_format', __( 'The zip file provided is not supported!', 'elementor-avator' ) );
	}
	...
	public function manage_columns( $columns ) {
		return [
			...
			'title' => __( 'Icon Set', 'elementor-avator' ),
			'icons_prefix' => __( 'CSS Prefix', 'elementor-avator' ),
		];
	}

	public function update_enter_title_here( $title, $post ) {
		if ( isset( $post->post_type ) && Icons_Manager::CPT === $post->post_type ) {
			return __( 'Enter Icon Set Name', 'elementor-avator' );
		}
	...
	public function register_ajax_actions( Ajax $ajax ) {
		$ajax->register_ajax_action( 'avator_assets_manager_custom_icon_upload', [ $this, 'custom_icons_upload_handler' ] );
	}
