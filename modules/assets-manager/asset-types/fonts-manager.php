<?php
namespace ElementorAvator\Modules\AssetsManager\AssetTypes;

use Elementor\Core\Common\Modules\Ajax\Module as Ajax;
use ElementorAvator\Plugin;
use ElementorAvator\Modules\AssetsManager\Classes;
...
class Fonts_Manager {
	 ...
	 * @param null $type
	 *
	 * @return array|bool|\ElementorAvator\Modules\AssetsManager\Classes\Font_Base
	 */
	...
	public function register_post_type_and_tax() {
		...
		$taxonomy_labels = [
			'name' => _x( 'Font Types', 'Font type taxonomy general name', 'elementor-avator' ),
			'singular_name' => _x( 'Font Type', 'Font type singular name', 'elementor-avator' ),
			'search_items' => __( 'Search Font Types', 'elementor-avator' ),
			'popular_items' => __( 'Popular Font Types', 'elementor-avator' ),
			'all_items' => __( 'All Font Types', 'elementor-avator' ),
			'edit_item' => __( 'Edit Font Type', 'elementor-avator' ),
			'update_item' => __( 'Update Font Type', 'elementor-avator' ),
			'add_new_item' => __( 'Add New Font Type', 'elementor-avator' ),
			'new_item_name' => __( 'New Font Type Name', 'elementor-avator' ),
			'separate_items_with_commas' => __( 'Separate Font Types with commas', 'elementor-avator' ),
			'add_or_remove_items' => __( 'Add or remove Font Types', 'elementor-avator' ),
			'choose_from_most_used' => __( 'Choose from the most used Font Types', 'elementor-avator' ),
			'not_found' => __( 'No Font Types found.', 'elementor-avator' ),
			'menu_name' => __( 'Font Types', 'elementor-avator' ),
		];
	...
	public function post_updated_messages( $messages ) {
		$messages[ self::CPT ] = [
			...
			1 => __( 'Font updated.', 'elementor-avator' ),
			2 => __( 'Custom field updated.', 'elementor-avator' ),
			3 => __( 'Custom field deleted.', 'elementor-avator' ),
			4 => __( 'Font updated.', 'elementor-avator' ),
			/* translators: %s: date and time of the revision */
			5 => isset( $_GET['revision'] ) ? sprintf( __( 'Font restored to revision from %s', 'elementor-avator' ), wp_post_revision_title( (int) $_GET['revision'], false ) ) : false,
			6 => __( 'Font saved.', 'elementor-avator' ),
			7 => __( 'Font saved.', 'elementor-avator' ),
			8 => __( 'Font submitted.', 'elementor-avator' ),
			9 => __( 'Font updated.', 'elementor-avator' ),
			10 => __( 'Font draft updated.', 'elementor-avator' ),
		];
	...
	public function register_admin_menu() {
		$menu_title = _x( 'Custom Fonts', 'Elementor Font', 'elementor-avator' );
		add_submenu_page(
			Settings::PAGE_ID,
			$menu_title,
			$menu_title,
	...
	public function update_enter_title_here( $title, $post ) {
		if ( isset( $post->post_type ) && self::CPT === $post->post_type ) {
			return __( 'Enter Font Family', 'elementor-avator' );
		}
	...
	public function manage_columns( $columns ) {
		return [
			...
			'title' => __( 'Font Family', 'elementor-avator' ),
			'font_preview' => __( 'Preview', 'elementor-avator' ),
		];
	}
	...
	public function register_ajax_actions( Ajax $ajax ) {
		$ajax->register_ajax_action( 'avator_assets_manager_panel_action_data', [ $this, 'assets_manager_panel_action_data' ] );
	}

	public function add_finder_item( array $categories ) {
		$categories['settings']['items']['custom-fonts'] = [
			'title' => __( 'Custom Fonts', 'elementor-avator' ),
			'icon' => 'typography',
	...
	protected function actions() {
		 ...
		 */
		do_action( 'elementor_avator/fonts_manager_loaded', $this );
	}
