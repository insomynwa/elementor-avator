<?php
namespace ElementorAvator\Modules\AssetsManager\Classes;
...
abstract class Assets_Base {
	...
	public function get_file_field( $field, $saved ) {
		...
		$html .= $this->get_input_field(
			[
				...
				'data-upload_text' => __( 'Upload', 'elementor-avator' ),
				'data-remove_text' => __( 'Delete', 'elementor-avator' ),
	...
	public function get_dropzone_field( $field ) {
		...
		$field['label'] = '<h4><span class="box__dragndrop">' . __( 'Drag & Drop to Upload', 'elementor-avator' ) . '</span></h4>';
		...
		<div class="elementor-dropzone-field">
			<div class="box__input">
				...
				<div class="elementor-button elementor--dropzone--upload__browse">
					<span><?php esc_html_e( 'Click here to browse', 'elementor-avator' ); ?></span>
				</div>
			</div>
			<div class="box__uploading"><?php esc_html_e( 'Uploading&hellip;', 'elementor-avator' ); ?></div>
			<div class="box__success"><?php esc_html_e( 'Done!', 'elementor-avator' ); ?></div>
			<div class="box__error"><?php esc_html_e( 'Error!', 'elementor-avator' ); ?> <span></span>.</div>
		</div>
		<?php
		return ob_get_clean();
	}
	...
	public function get_repeater_field( $field, $saved ) {
		...
		$add_label = isset( $field['add_label'] ) ? $field['add_label'] : __( 'Add item', 'elementor-avator' );
		$row_label = isset( $field['row_label'] ) ? $field['row_label'] : __( 'Row', 'elementor-avator' );
		$row_label_html_args = [
			'id' => 'row_label_' . $js_id,
	...
	private function get_repeater_tools( $field ) {
		$confirm = isset( $field['confirm'] ) ? $field['confirm'] : __( 'Are you sure?', 'elementor-avator' );
		$remove_title = isset( $field['remove_title'] ) ? $field['remove_title'] : __( 'Delete', 'elementor-avator' );
		$toggle_title = isset( $field['toggle_title'] ) ? $field['toggle_title'] : __( 'Edit', 'elementor-avator' );
		$close_title = isset( $field['close_title'] ) ? $field['close_title'] : __( 'Close', 'elementor-avator' );
