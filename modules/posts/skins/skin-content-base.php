...
use ElementorAvator\Modules\ThemeBuilder\Module as ThemeBuilder;
...
trait Skin_Content_Base {
	...
	public function get_title() {
		return __( 'Full Content', 'elementor-avator' );
	}
	...
	public function register_thumbnail_controls() {
		$this->add_control(
			'thumbnail',
			[
				'label' => __( 'Show Thumbnail', 'elementor-avator' ),
	...
	public function render_post_content( $with_wrapper = false ) {
		...
		if ( Plugin::elementor()->preview->is_preview_mode( $post->ID ) ) {
			...
			if ( empty( $content ) ) {
				...
				wp_link_pages( [
					'before' => '<div class="page-links elementor-page-links"><span class="page-links-title elementor-page-links-title">' . __( 'Pages:', 'elementor-avator' ) . '</span>',
					'after' => '</div>',
					'link_before' => '<span>',
					'link_after' => '</span>',
					'pagelink' => '<span class="screen-reader-text">' . __( 'Page', 'elementor-avator' ) . ' </span>%',
