<?php
namespace ElementorAvator\Modules\Woocommerce;

use Elementor\Core\Documents_Manager;
use Elementor\Settings;
use ElementorAvator\Base\Module_Base;
use ElementorAvator\Modules\ThemeBuilder\Classes\Conditions_Manager;
use ElementorAvator\Modules\Woocommerce\Conditions\Woocommerce;
use ElementorAvator\Modules\Woocommerce\Documents\Product;
use ElementorAvator\Modules\Woocommerce\Documents\Product_Post;
use ElementorAvator\Modules\Woocommerce\Documents\Product_Archive;
...
class Module extends Module_Base {
	...
	public function register_tags() {
		...
		$module->register_group( self::WOOCOMMERCE_GROUP, [
			'title' => __( 'WooCommerce', 'elementor-avator' ),
		] );

		foreach ( $tags as $tag ) {
			$module->register_tag( 'ElementorAvator\\Modules\\Woocommerce\\tags\\' . $tag );
		}
	}
		...
		<div class="elementor-menu-cart__toggle elementor-button-wrapper">
			<a id="elementor-menu-cart__toggle_button" href="#" class="elementor-button elementor-size-sm">
				...
				<span class="elementor-button-icon" <?php echo $counter_attr; ?>>
					...
					<span class="elementor-screen-only"><?php esc_html_e( 'Cart', 'elementor-avator' ); ?></span>
				</span>
			</a>
		</div>
	...
	public function register_admin_fields( Settings $settings ) {
		$settings->add_section( Settings::TAB_INTEGRATIONS, 'woocommerce', [
			'callback' => function() {
				echo '<hr><h2>' . esc_html__( 'WooCommerce', 'elementor-avator' ) . '</h2>';
			},
			'fields' => [
				self::OPTION_NAME_USE_MINI_CART => [
					'label' => __( 'Mini Cart Template', 'elementor-avator' ),
					'field_args' => [
						'type' => 'select',
						'std' => 'initial',
						'options' => [
							'initial' => '', // Relevant until either menu-cart widget is used or option is explicitly set to 'no'.
							'no' => __( 'Disable', 'elementor-avator' ),
							'yes' => __( 'Enable', 'elementor-avator' ),
						],
						'desc' => __( 'Set to `Disable` in order to use your Theme\'s or WooCommerce\'s mini-cart template instead of Elementor\'s.', 'elementor-avator' ),
					],
				],
			],
		] );
	...
	public function __construct() {
		...
		add_filter( 'elementor_avator/frontend/localize_settings', [ $this, 'localized_settings_frontend' ] );
