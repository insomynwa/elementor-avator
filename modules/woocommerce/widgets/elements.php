...
use ElementorAvator\Modules\QueryControl\Module as QueryModule;
...
class Elements extends Widget_Base {
	...
	public function get_title() {
		return __( 'WooCommerce Pages', 'elementor-avator' );
	}
	...
	protected function _register_controls() {
		...
		$this->add_control(
			'element',
			[
				'label' => __( 'Page', 'elementor-avator' ),
				'type' => Controls_Manager::SELECT,
				'options' => [
					'' => '— ' . __( 'Select', 'elementor-avator' ) . ' —',
					'woocommerce_cart' => __( 'Cart Page', 'elementor-avator' ),
					'product_page' => __( 'Single Product Page', 'elementor-avator' ),
					'woocommerce_checkout' => __( 'Checkout Page', 'elementor-avator' ),
					'woocommerce_order_tracking' => __( 'Order Tracking Form', 'elementor-avator' ),
					'woocommerce_my_account' => __( 'My Account', 'elementor-avator' ),
				],
			]
		);
		...
		$this->add_control(
			'product_id',
			[
				'label' => __( 'Product', 'elementor-avator' ),
		...
		switch ( $settings['element'] ) {
			...
			case 'product_page':
				...
				if ( empty( $product ) && current_user_can( 'manage_options' ) ) {
					return __( 'Please set a valid product', 'elementor-avator' );
				}
	...
	protected function render() {
		...
		if ( 'woocommerce_checkout' === $this->get_settings( 'element' ) && '<div class="woocommerce"></div>' === $html ) {
			$html = '<div class="woocommerce">' . __( 'Your cart is currently empty.', 'elementor-avator' ) . '</div>';
		}
