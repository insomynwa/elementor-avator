/*! elementor-avator - v2.7.3 - 28-10-2019 */
...
/******/ 	// mode & 2: merge all propertyes of value into the ns
		...
		value: function onRender() {
			...
			this.children.each(function (child) {
				...
				var $imageWrapper = jQuery('<div>', { class: 'elementor-popup__display-settings_controls_group__icon' }),
				    $image = jQuery('<img>', { src: elementorAvator.config.urls.modules + ('popup/assets/images/' + name + '/' + groupName + '.svg') });
...
module.exports = elementor.modules.controls.Repeater.extend({
	...
	initialize: function initialize() {
		...
		this.config = elementorAvator.config.theme_builder;
			...
			value: function addPublishTabs() {
				...
				jQuery.each(this.displaySettingsTypes, function (type, data) {
					...
					component.addTab(type, {
						...
						description: elementorAvator.translate('popup_publish_screen_' + type + '_description'),
						image: elementorAvator.config.urls.modules + ('popup/assets/images/' + type + '-tab.svg')
					});
				});
			}
		}, {
...
var _class = function (_elementorModules$edi) {
	...
	_createClass(_class, [{
		...
		value: function onElementorPanelInit() {
			...
			elementorAvator.modules.themeBuilder.on('publish-layout:init', this.addDisplaySettingsPublishScreens.bind(this));
		...
		value: function onPanelFooterSubmenuItemClick(type) {
			elementorAvator.modules.themeBuilder.showPublishModal();

			elementorAvator.modules.themeBuilder.getPublishLayout().modalContent.currentView.showScreenByName(type);
		}
	}]);
...
if (!elementorAvatorEditorConfig.useComponentsRouter) {
	...
	module.exports = elementorModules.editor.utils.Module.extend({
		...
		onElementorInit: function onElementorInit() {
			if (!elementorAvator.config.theme_builder) {
				return;
			}
		...
		setConditionsModel: function setConditionsModel() {
			var themeBuilderModuleConfig = elementorAvator.config.theme_builder,
		...
		onEditorSave: function onEditorSave() {
			...
			elementorAvator.ajax.addRequest('theme_builder_save_conditions', {
				...
				success: function success() {
					elementorAvator.config.theme_builder.settings.conditions = _this.conditionsModel.get('conditions');
				}
			});
		}
	});
...
module.exports = elementorModules.editor.utils.Module.extend({
	...
	onEditorSave: function onEditorSave() {
		...
		elementorAvator.ajax.addRequest('theme_builder_save_conditions', {
			data: this.conditionsModel.toJSON({ remove: ['default'] }),
			success: function success() {
				elementorAvator.config.theme_builder.settings.conditions = _this2.conditionsModel.get('conditions');
			}
		});
	}
});
...
module.exports = SaverBehavior.extend({
	...
	toggleMenuConditions: function toggleMenuConditions() {
		this.ui.menuConditions.toggle(!!elementorAvator.config.theme_builder.settings.location);
	},
	...
	onRender: function onRender() {
		...
		this.ui.menuConditions = this.view.addSubMenuItem('saver-options', {
			...
			callback: function callback() {
				return elementorAvator.modules.themeBuilder.showPublishModal();
			}
		});
	...
	onChangeLocation: function onChangeLocation(settings) {
		if (!_.isUndefined(settings.changed.location)) {
			elementorAvator.config.theme_builder.settings.location = settings.changed.location;
			this.toggleMenuConditions();
		}
	},

	onClickButtonPublish: function onClickButtonPublish() {
		var hasConditions = elementorAvator.config.theme_builder.settings.conditions.length,
		    hasLocation = elementorAvator.config.theme_builder.settings.location,
