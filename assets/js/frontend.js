...
/***/ (function(module, exports, __webpack_require__) {
	...
	var ElementorAvatorFrontend = function (_elementorModules$Vie) {
		_inherits(ElementorAvatorFrontend, _elementorModules$Vie);
	
		function ElementorAvatorFrontend() {
			_classCallCheck(this, ElementorAvatorFrontend);
	
			return _possibleConstructorReturn(this, (ElementorAvatorFrontend.__proto__ || Object.getPrototypeOf(ElementorAvatorFrontend)).apply(this, arguments));
		}
	
		_createClass(ElementorAvatorFrontend, [{
			key: 'onInit',
			value: function onInit() {
				_get(ElementorAvatorFrontend.prototype.__proto__ || Object.getPrototypeOf(ElementorAvatorFrontend.prototype), 'onInit', this).call(this);
	
				this.config = ElementorAvatorFrontendConfig;
		...
		return ElementorAvatorFrontend;
	}(elementorModules.ViewModule);
	
	window.elementorAvatorFrontend = new ElementorAvatorFrontend();
	...
	var _class = function (_elementorModules$Mod) {
		...
		function _class() {
			...
			elementorAvatorFrontend.modules.linkActions.addAction('popup:open', _this.showPopup.bind(_this));
	
			elementorAvatorFrontend.modules.linkActions.addAction('popup:close', _this.closePopup.bind(_this));
			...
			value: function showModal(avoidMultiple) {
				...
				if (!this.isEdit) {
					if (!elementorFrontend.isWPPreviewMode()) {
						...
						if (avoidMultiple && elementorAvatorFrontend.modules.popup.popupPopped && settings.avoid_multiple_popups) {
							return;
						}
					}
				...
				elementorAvatorFrontend.modules.popup.popupPopped = true;
			}
		}, {
	...
	var PopupFormActions = elementorModules.frontend.handlers.Base.extend({
		...
		handleFormAction: function handleFormAction(event, response) {
			...
			if ('open' === popupSettings.action) {
				return elementorAvatorFrontend.modules.popup.showPopup(popupSettings);
			}
			setTimeout(function () {
				return elementorAvatorFrontend.modules.popup.closePopup(popupSettings, event);
			}, 1000);
		}
	});
	...
	var TOCHandler = function (_elementorModules$fro) {
		...
		_createClass(TOCHandler, [{
			...
			value: function handleNoHeadingsFound() {
				if (elementorFrontend.isEditMode()) {
					return this.elements.$tocBody.html(elementorAvator.translate('toc_no_headings_found'));
				}
			}
		}, {
			key: 'collapseOnInit',
	...
	module.exports = elementorModules.frontend.handlers.Base.extend({
		...
		getDefaultSettings: function getDefaultSettings() {
			return {
				...
				action: 'elementor_avator_forms_send_form',
				ajaxUrl: elementorAvatorFrontend.config.ajaxurl
			};
		},
	...
	var config = ElementorAvatorFrontendConfig.facebook_sdk,
	...
	var MiniCartHandler = elementorModules.frontend.handlers.Base.extend({
		...
		bindEvents: function bindEvents() {
			...
			$toggle.on('click', function (event) {
				var noQueryParams = -1 === ElementorAvatorFrontendConfig.menu_cart.cart_page_url.indexOf('?'),
					currentUrl = noQueryParams ? window.location.origin + window.location.pathname : window.location.href,
					isCart = ElementorAvatorFrontendConfig.menu_cart.cart_page_url === currentUrl,
					isCheckout = ElementorAvatorFrontendConfig.menu_cart.checkout_page_url === currentUrl;
	
				if (!isCart && !isCheckout) {
					event.preventDefault();
					$container.toggleClass(classes.isShown);
				} else {
					var cartUrl = ElementorAvatorFrontendConfig.menu_cart.cart_page_url;
